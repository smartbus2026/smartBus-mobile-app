...
function StudentBottomBar() {
  ...
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: colors.card, borderTopWidth: 1, borderTopColor: colors.border, height: BOTTOM_BAR_HEIGHT, paddingBottom: Platform.OS === 'ios' ? 25 : 30, paddingTop: 20 }}>
      {tabs.map((tab) => {
        const Icon = tab.icon;
