...
function calcWindow(settings: BookingSettings): WindowState {
  ...
  if (closeMinutes >= openMinutes) {
    ...
    isOpen = currentMinutes >= openMinutes || currentMinutes < closeMinutes;
    if (isOpen) {
      minutesRemaining = currentMinutes >= openMinutes
        ? (1440 - currentMinutes) + closeMinutes
    ...
    <View style={{ marginBottom: 16 }}>
      ...
      {isOpen && (
        <View style={[s.dropdown, { backgroundColor: colors.background, borderColor: colors.border }]}>
          {options.length === 0 ? (
            <Text style={[s.dropdownItemTxt, { color: colors.icon, padding: 16, textAlign: 'center' }]}>
...
export default function BookTripScreen() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setModal({ open: true, type: 'success', msg: res?.data?.message || t('booking_success_msg') });
      setSelectedRouteId('');
      setSelectedTimeSlot('');
      setSelectedSpecificReturn('');
    } catch (err: any) {
      setModal({ open: true, type: 'error', msg: err?.response?.data?.message || t('booking_error_msg') });
    } finally {
      setIsBooking(false);
    }
  };
  ...
  if (isLoading) {
    return (
      <View style={[s.center, { backgroundColor: colors.background }]}>
        ...
        <Text style={[s.loadingTxt, { color: colors.icon }]}>{t('loading_system').toUpperCase()}</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[s.root, { backgroundColor: colors.background }]}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {/* Header */}
        <View style={s.header}>
          <Text style={[s.headerTitle, { color: colors.text }]}>{t('book_route').toUpperCase()}</Text>
          <Text style={[s.headerSub, { color: colors.icon }]}>{t('book_route_subtitle')}</Text>
        </View>
        ...
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={s.sectionHeader}>
            ...
            <Text style={[s.sectionLabel, { color: colors.tint }]}>{t('select_route').toUpperCase()}</Text>
          </View>
          <CustomSelectBox
        ...
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={s.sectionHeader}>
            ...
            <Text style={[s.sectionLabel, { color: colors.text }]}>{t('select_time_slot').toUpperCase()}</Text>
          </View>
            ...
            {['Morning', 'Return'].map(slot => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={[s.slotTxt, { color: colors.icon }, isSel && { color: colors.tint }]}>
                    {slot === 'Morning' ? t('morning').toUpperCase() : t('return').toUpperCase()}
                  </Text>
                </TouchableOpacity>
              );
            })}
        ...
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border, overflow: 'hidden' }]}>
          ...
          <View style={s.cardTopRow}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              ...
              <Text style={[s.sectionLabel, { color: colors.tint }]}>{t('booking_window').toUpperCase()}</Text>
            </View>
            <View style={[s.badge, windowState.isOpen ? s.badgeOpen : s.badgeClosed]}>
              <Text style={[s.badgeTxt, windowState.isOpen ? { color: '#22c55e' } : { color: '#ef4444' }]}>
                {windowState.isOpen ? t('open').toUpperCase() : t('closed').toUpperCase()}
              </Text>
            </View>
          </View>
          ...
          <View style={s.winRow}>
            <View>
              <Text style={[s.winState, { color: colors.icon }]}>
                {windowState.isOpen ? t('time_remaining').toUpperCase() : t('currently_closed').toUpperCase()}
              </Text>
        ...
        >
          {isBooking ? (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              {!windowState.isOpen
                ? <><Lock size={16} color={colors.icon} /><Text style={[s.confirmTxt, { color: colors.icon }]}>{t('booking_closed').toUpperCase()}</Text></>
                : <><Bus  size={16} color="#000"         /><Text style={s.confirmTxt}>{t('register_booking_demand').toUpperCase()}</Text></>
              }
            </View>
          )}
        </TouchableOpacity>
        ...
        <View style={s.overlay}>
          <View style={[s.modalCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <Text style={[s.modalTitle, { color: colors.text }]}>
              {modal.type === 'success' ? t('demand_registered').toUpperCase() : t('action_failed').toUpperCase()}
            </Text>
            ...
            <TouchableOpacity
              style={[
                s.modalBtn,
                {
                  backgroundColor: modal.type === 'success' ? colors.tint : colors.background,
                  borderColor: modal.type === 'success' ? 'transparent' : 'rgba(239,68,68,0.3)',
                  borderWidth: 1,
                },
              ]}
              onPress={() => {
            ...
            >
              <Text style={[s.modalBtnTxt, modal.type === 'error' && { color: '#ef4444' }]}>
                {modal.type === 'success' ? t('view_my_trips').toUpperCase() : t('close_try_again').toUpperCase()}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
