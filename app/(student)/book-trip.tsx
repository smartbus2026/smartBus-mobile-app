...
// ── Section Header ────────────────────────────────────────────────────────────
const SectionLabel = ({ icon, text, tint }: { icon: string; text: string; tint: string }) => (
  <Text style={[s.sectionLabel, { color: tint }]}>{icon}  {text}</Text>
);
      ...
      >
        ...
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={s.cardTopRow}>
            <SectionLabel icon="📅" text="BOOKING WINDOW" tint={colors.tint} />
        ...
        {/* ── Select Route ── */}
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <SectionLabel icon="🚌" text="SELECT ROUTE" tint={colors.tint} />
        ...
        {/* ── Select Pickup ── */}
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <SectionLabel icon="📍" text="SELECT PICKUP POINT" tint={colors.tint} />
          {!selectedTripId ? (
        ...
        {/* ── Select Return ── */}
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <SectionLabel icon="🔄" text="SELECT RETURN TIME" tint={colors.tint} />
          {!selectedTripId ? (
            <Text style={[s.emptyTxt, { color: colors.icon }]}>Please select a route first</Text>
          ) : returns.length === 0 ? (
        ...
        >
          {isBooking
            ...
            : <Text style={[s.confirmTxt, !canBook && { color: colors.icon }]}>
...
const s = StyleSheet.create({
  ...
  sectionLabel:       { fontSize: 10, fontWeight: '700', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 14 },
