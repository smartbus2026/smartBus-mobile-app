...
const TripCard = ({ trip, colors }: { trip: DriverTrip; colors: any }) => {
  ...
  const departureTime = (() => {
    try {
      return new Date(trip.scheduled_time ?? trip.date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
    } catch { return '—'; }
  })();
  ...
  return (
    <View style={[s.tripCard, { backgroundColor: colors.card, borderColor: isThisActive ? 'rgba(34,197,94,0.35)' : colors.border }]}>
      ...
      <View style={s.tripCardInner}>
        ...
        <View style={s.tripCardHeader}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[s.tripRouteName, { color: colors.text }]} numberOfLines={1}>{trip.route?.name ?? '—'}</Text>
            <Text style={[s.tripStopsLine, { color: colors.icon }]} numberOfLines={1}>{firstStop} → {lastStop}</Text>
          </View>
        ...
        <View style={[s.infoBlock, { borderColor: colors.border }]}>
          ...
          <InfoRow icon={<Clock size={13} color="#f7a01b" />}    label={t('departure').toUpperCase()}  value={departureTime}                                 colors={colors} />
            ...
            <View style={s.stopsList}>
              ...
              {stops.map((stop, idx) => (
                <View key={stop._id ?? idx} style={s.stopRow}>
                  ...
                  {idx === 0 && <Text style={s.stopTag}>{t('stop_start').toUpperCase()}</Text>}
        ...
        {isThisActive && (
          <View style={[s.gpsBar, { backgroundColor: 'rgba(34,197,94,0.06)', borderColor: 'rgba(34,197,94,0.2)' }]}>
            ...
            <Text style={s.gpsBroadcastTxt}>{t('gps_broadcasting_live').toUpperCase()}</Text>
            {geo.lat !== null && (
        ...
        {trip.status === 'scheduled' && (
          <View style={s.actionBlock}>
            <TouchableOpacity
              ...
              style={[s.actionBtn, startDisabled ? [s.actionBtnDisabled, { backgroundColor: colors.background, borderColor: colors.border }] : s.actionBtnStart]}
              activeOpacity={0.8}
            >
              {isBtnLoading
            ...
            {!canStart && <Text style={[s.actionHint, { color: colors.icon }]}>{t('unlocks_one_hour_before')}</Text>}
            {!!activeTrip && !isThisActive && <Text style={[s.actionHint, { color: '#ef4444' }]}>{t('end_active_first')}</Text>}
          </View>
        )}
...
export default function DriverDashboardScreen() {
  ...
  return (
    <View style={[s.root, { backgroundColor: colors.background }]}>
      <TopBar title={t('dashboard')} showMenu showSettings onSettingsPress={() => router.push('/(driver)/settings' as any)} />
      ...
      >
        ...
        <View style={s.pageHeader}>
          ...
          <View>
            <Text style={[s.pageTitle, { color: colors.text }]}>
              ...
              <Text style={{ color: colors.tint }}>{t('dashboard').toUpperCase()}</Text>
            </Text>
            <Text style={[s.pageSubtitle, { color: colors.icon }]}>{t('driver_command_center').toUpperCase()}</Text>
          </View>
        </View>
            ...
            {geo.error ? (
              ...
              <TouchableOpacity onPress={() => router.push('/(driver)/live-tracking' as any)}>
                <View style={s.gpsViewMapBtn}>
                  ...
                  <Text style={s.gpsViewMapTxt}>{t('view_on_map').toUpperCase()}</Text>
                </View>
              </TouchableOpacity>
            ) : (
              <Text style={[s.gpsAcquiringTxt, { color: colors.icon }]}>{t('acquiring_gps_signal')}</Text>
            )}
          </View>
        )}
        ...
        <View style={s.statsRow}>
          {[
            { label: t('total_trips').toUpperCase(),  value: total,     color: colors.tint, bg: `${colors.tint}1A` },
            { label: t('status_scheduled').toUpperCase(), value: scheduled, color: '#60a5fa', bg: 'rgba(96,165,250,0.1)' },
            { label: t('active').toUpperCase(),       value: active,    color: '#22c55e',   bg: 'rgba(34,197,94,0.1)' },
            { label: t('completed').toUpperCase(),    value: completed, color: '#f7a01b',   bg: 'rgba(247,160,27,0.1)' },
          ].map(st => (
        ...
        <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <View style={s.systemRow}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={[s.systemSub, { color: colors.icon }]}>{t('system_status').toUpperCase()}</Text>
              <Text style={[s.systemTitle, { color: colors.text }]}>{t('all_operations_normal').toUpperCase()}</Text>
            </View>
            <View style={s.pulseDot} />
          </View>
          <Text style={[s.systemNote, { color: colors.icon }]}>
            {t('gps_broadcasting_live')}{' '}
            <Text style={{ color: colors.tint, fontWeight: '800' }}>30</Text>{' '}
            {t('min')}
          </Text>
        </View>
